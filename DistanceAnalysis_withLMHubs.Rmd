---
title: "Distance_Analysis_withSharedLM"
output:
  pdf_document:
    toc: yes
  html_notebook:
...
#This notebook conducts the link distance analysis, without excluding hubs (requested by reviewer)
...
ac.distances.2.hpc_all <- distance.ac.123.copy %>%
  filter(roi %in% hpcROIs, stim1_type == "LM",distance_comparison %in% acdists,sesNum == "2")
...
Next, look at across track link distance 2 3 4
...
contrasts.wc.distances.123.vmpfc
```
...
```{r, fig.height=2.5, fig.width=3}
ses <- c("Pre-Learning", "Post Local Navigation", "Post Global Navigation")
concodingLM <- summary(contrasts.wc.distances.123.vmpfc$contrasts) %>%
  filter(contrast == "1 1 - 1 2" | contrast == "2 1 - 2 2" | contrast == "3 1 - 3 2")
concodingLM$sesNum <- ses
concodingLM$sesNum <- factor(concodingLM$sesNum, levels = c("Pre-Learning", "Post Local Navigation", "Post Global Navigation"))

vmpfc_withindist_contrasts <- ggplot() + 
  geom_bar(data = concodingLM, aes(y=estimate,x=sesNum,fill=sesNum),stat = "identity") + 
  geom_errorbar(data = concodingLM,aes(y=estimate, x=sesNum, fill=sesNum,ymin=estimate-SE,ymax=estimate+SE), width=0.2, color = "black") +
  geom_hline(yintercept = 0, alpha = 0.2) +
  ggtitle(label = 'Contrast Estimates', subtitle = sprintf(vmpfc)) + 
  scale_fill_manual(values = colors123)+
  scale_color_manual(values = colors123)+
  theme_classic()+
  theme(plot.title = element_text(hjust=0.5, size = 14), 
  ...
  ylab("Link Distance 1 - Link Distance 2 Similarity") +
  xlab("")
vmpfc_withindist_contrasts
ggsave(filename = '~/Dropbox/Project/writing/HPC_Paper/figures/vmpfc_withindist_contrasts.png', vmpfc_withindist_contrasts, dpi=600)
```

Next, look at across tracks, link distances 2 3 4
model all 3 days- distance, continuous variable
```{r}
...
contrasts.wc.distances.123.calc
```
```{r}
...
```{r, fig.height=2.5, fig.width=3}
ses <- c("Pre-Learning", "Post Local Navigation", "Post Global Navigation")
concodingLM <- summary(contrasts.wc.distances.123.calc$contrasts) %>%
  filter(contrast == "1 1 - 1 2" | contrast == "2 1 - 2 2" | contrast == "3 1 - 3 2")
concodingLM$sesNum <- ses
concodingLM$sesNum <- factor(concodingLM$sesNum, levels = c("Pre-Learning", "Post Local Navigation", "Post Global Navigation"))

control_localdist <- ggplot() + 
  geom_bar(data = concodingLM, aes(y=estimate,x=sesNum,fill=sesNum),stat = "identity") + 
  geom_errorbar(data = concodingLM,aes(y=estimate, x=sesNum, fill=sesNum,ymin=estimate-SE,ymax=estimate+SE), width=0.2, color = "black") +
  geom_hline(yintercept = 0, alpha = 0.2) +
  ggtitle(label = 'Contrast Estimates', subtitle = sprintf("Calcarine")) + 
  scale_fill_manual(values = colors123)+
  scale_color_manual(values = colors123)+
  theme_classic()+
  theme(plot.title = element_text(hjust=0.5, size = 14), 
  ...
  xlab("")
control_localdist
#ggsave(filename = '~/Dropbox/Project/writing/HPC_Paper/figures/control_within_distance.png', control_localdist, dpi=600)
```
...
```{r, fig.height=2.5, fig.width=3}
ses <- c("Pre-Learning", "Post Local Navigation", "Post Global Navigation")
concodingLM <- summary(contrasts.wc.distances.123.erc_left$contrasts) %>%
  filter(contrast == "1 1 - 1 2" | contrast == "2 1 - 2 2" | contrast == "3 1 - 3 2")
concodingLM$sesNum <- ses
concodingLM$sesNum <- factor(concodingLM$sesNum, levels = c("Pre-Learning", "Post Local Navigation", "Post Global Navigation"))

ercL_within_dist <- ggplot() + 
  geom_bar(data = concodingLM, aes(y=estimate,x=sesNum,fill=sesNum),stat = "identity") + 
  geom_errorbar(data = concodingLM,aes(y=estimate, x=sesNum, fill=sesNum,ymin=estimate-SE,ymax=estimate+SE), width=0.2, color = "black") +
  geom_hline(yintercept = 0, alpha = 0.2) +
  ggtitle(label = 'Contrast Estimates', subtitle = sprintf("Left Entorhinal Cortex")) + 
  scale_fill_manual(values = colors123)+
  scale_color_manual(values = colors123)+
  theme_classic()+
  ylim(-0.05,0.051) +
  ...
  xlab("")
ercL_within_dist
#ggsave(filename = '~/Dropbox/Project/writing/HPC_Paper/figures/ercL_within_dist.png', ercL_within_dist, dpi=600)
```
